use crate::{config::AppConfig, error::{AppError, AppResult}, model::{commit::ReviewCommit, issue::ReviewIssue, task::ReviewTask}};
use lettre::{message::Mailbox, AsyncSmtpTransport, AsyncTransport, Message, Tokio1Executor};
use std::collections::BTreeSet;

pub async fn send_alert(cfg: &AppConfig, task: &ReviewTask, commits: &[ReviewCommit], issues: &[ReviewIssue], owner_email: Option<&str>) -> AppResult<bool> {
    if !cfg.mail.enabled {
        return Ok(false);
    }
    ...
    for c in commits {
        if let Some(email) = &c.author_email {
            if !email.trim().is_empty() { recipients.insert(email.trim().to_string()); }
        }
    }
    ...
    let subject = format!("【AI代码审核告警】【{}】{}/{} 检测到严重问题", task.risk_level.clone().unwrap_or_default(), task.repo_name, task.branch_name);
    let body = format!(
        "仓库: {}\n分支: {}\nold_commit: {}\nnew_commit: {}\n提交人: {}\nHIGH: {}\nCRITICAL: {}\n详情: http://{}:{}/tasks/{}\n\n问题摘要:\n{}",
        task.repo_name,
        task.branch_name,
    ...
    let mut builder = Message::builder()
        .from(cfg.mail.from.parse::<Mailbox>().map_err(|e| AppError::Other(format!("invalid mail.from: {e}")))?)
        .subject(subject);
    for r in recipients {
    ...
    let creds = lettre::transport::smtp::authentication::Credentials::new(cfg.mail.username.clone(), cfg.mail.password.clone());
    let mailer = AsyncSmtpTransport::<Tokio1Executor>::relay(&cfg.mail.smtp_host)
